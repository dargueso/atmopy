...
def plevs_interp(path_in,path_out,path_geo,syear,eyear,smonth,emonth,plevs,patt,patt_wrf,dom,wrun,varn):
    ...
    while (y < eyear or (y == eyear and m <= emonth)):
        ...
        for thour in range(len(filesin_wrf)):
            ...
            zFragment = wrf.vinterp(fwrf,field,vert_coord='pressure',interp_levels=plevs)
...
def create_zlevs_netcdf(var,filename):
    ...
    setattr(outlev,"long_name","height_above_mean_sea_level")
...
###########################################################
###########################################################
def zlevs_interp(path_in,path_out,path_geo,path_proc,syear,eyear,smonth,emonth,zlevs,patt,patt_wrf,dom,wrun,varn):
    ...
    while (y < eyear or (y == eyear and m <= emonth)):
        ...
        filesin_wrf = sorted(glob('%s/%s/out/%s_%s_%s*' %(path_in,wrun,patt_wrf,dom,sdate)))
        filesin_pp = xr.open_dataset("%s/%s/%s-%s/%s_01H_%s_%s.nc" %(path_proc, wrun,syear,eyear,patt,varn,sdate[:7]))

        field = filesin_pp[varn].sel(time=sdate,method='nearest').values
        ...
        for thour in range(len(filesin_wrf)):
            ...
            fwrf.variables['F']=geofile.variables['F']
            tFragment = wrftime2date(filesin_wrf[thour].split())[:]
            zFragment = wrf.vinterp(fwrf,field[thour,:,:,:],vert_coord='ght_msl',interp_levels=zlevs)
        ...
        varinfo = { 'values': fieldint,
                    'varname': varn,
                    'zlevs': zlevs,
                    'atts':filesin_pp[varn].attrs,
                    'lat': filesin_pp.lat.values,
                    'lon': filesin_pp.lon.values,
                    'times': otimes}

        fileout = "%s/%s_HGT_%s_%s.nc" %(fullpathout,patt,varn,sdate)
